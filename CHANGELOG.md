...
## [v8.3.3-BETA] — 2026-06-22 · доставка обновлений (bump версий плагинов)

### Fixed
- **Критично: апдейт не доходил до пользователей.** Изменения BETA-релиза (#19 дедуп команд↔скиллов + удаление вложенного `marketplace.json`, #20 ALPHA→BETA) меняли *содержимое* плагина, но **версия в `plugin.json` оставалась `8.3.2-C` / `8.3.1-L`**. Версия — «пин»: Claude Code видит ту же строку → `/plugin update` считает «актуально» и отдаёт старую закэшированную (альфа) копию. Симптом: на GitHub папка плагина свежая, а установленный плагин — старее.
- **Бамп версий, чтобы доставить накопленные изменения:** cloud `p2p-v8c3` `8.3.2-C` → **`8.3.3-C`**; light `p2p-v8l3` `8.3.1-L` → **`8.3.2-L`**. Теперь `/plugin update p2p-v8c3@p2p` подтянет BETA-содержимое.

> Пользователям: `/plugin marketplace update p2p` → `/plugin update p2p-v8c3`. Застрявшим на альфе (десктоп-маркетплейс `local-desktop-app-uploads`) — скрипт `tools/p2p-clean.*` + чистая установка.
