...
## [8.4.8] — 2026-09-10 · Данные выправлены по эталону, плагин догнал чат-форму, меню H без тихих отказов

### Факты 8.7.4 (срез 2026-09-10)
- **GPT-6 Astra** (`gpt-6-astra`, GA 09.09) внесён во все пять тел: 1 050 000 / 128 000,
  $10 / $1 cached / $12.50 write / $50, cutoff 2026-04-30, effort low..max. Порог 272K действует
  вместе с кэшем (×2 вход и кэш, ×1.5 выход) — контекст держать ниже. Инструменты только через
  `/v1/responses`. Primary `webdev / frontend_generation` в H и N, в C — operator-explicit.
- **Вендор Grok — SpaceXAI (бывш. xAI)**; продуктовое имя и api-строки прежние, распознавание
  хоста дополнено `spacexai` и НЕ потеряло прежние написания.
- **GLM-5.3-Flash** — промо истекло 09.09: $0.15 / $0.03 / $0.50.
- **Qwen3.8-Max** — max output 131 072; тариф плоский 12/36 CNY, $2/$6 — приближение.
- **MiniMax Token Plan** — Plus $22 / Max $55 / Ultra $132; модель track-only.
- **Claude Code 2.1.263…2.1.267** — `maxEffortLevel` (при корпоративном потолке побеждает нижний),
  хуки `PreModelSwitch` (отказ при неопределимом наборе хуков плагина),
  `--system-prompt-snapshot off`, `model` во frontmatter скилла = смена модели и чтение
  истории без кэша, алиас `fable` → Fable 5 в Claude apps gateway, пол версии v2.1.257,
  коды 404/400 (не 403), сужение token counting, per-message effort на Google Cloud.
- **Правило G1 переписано**: `temperature` / `top_p` / `top_k` объявлены deprecated для всей
  линии Gemini 3.x с 21.07.2026; для Deep Think temperature не задавать, глубина —
  `thinking_level`. Гарантия «HTTP 400» снята: источник её больше не подтверждает.
- **Gemini 3.5 Pro** — пятый пропущенный срок GA. **Grok 4.7** объявлен на 12.09 без id, цены
  и контекста — не пре-маршрутизируется. **deepseek-v4.1-flash** в BASE и маршруты не вносится.
- **Ревизия G22 для Astra**: запрет Sol-образца не переносится и G22 не снимается — Astra
  допускается с проверкой на уровне результата (диффы, прогоны тестов, ворота разрешений).

### Что было сломано у нас и как исправлено
- Перезалив 8.4.7-L с концами строк CRLF 10.09 сломал загрузку модулей на части хостов
  (`\r` в хвосте адреса из индекса). Ассет возвращён к опубликованному в тот же день,
  тег и история не трогались. В 8.4.8 сборка идёт из тега, концы строк проверяет карантин
  отдельной байтовой проверкой.
- Прежний чек-лист не имел ни одного проверяемого пункта, и 8.4.7 вышел мимо него. Публикацию
  теперь запрещает `tools/release_gate.py` (15 проверок, у каждой число осмотренных объектов;
  ноль осмотренных — провал) с парой-слепотой.
- Вшитый `live_specs.md` (62 КБ × 4) убран из сборок: факты в BASE, свежее — по LIVE-каналу.
- Модули Lite публикуются отдельным гистом на каждый выпуск.
- Ограничение, названное вслух: у уже установленного 8.4.7 новые МАРШРУТЫ по LIVE-каналу
  не появятся — форма DELTA несёт цены, статусы и дедлайны, но не таблицу маршрутов.

### Context
Разбор 09–10.09 нашёл тридцать четыре расхождения между четырьмя редакциями и эталонными
live specs. Двенадцать из них устроены одинаково: факт исправлен в файле-объявлении
(`_live/MANIFEST.md`, `!!db_*`) и не исправлен у потребителей (`vendors/*`, `_live/live_core.md`,
модули Lite). Самое тяжёлое — отменённое подорожание Sonnet 5: дата 01.09 наступила,
и сборка называла вдвое завышенную цену на модель по умолчанию Free/Pro, при том что
её собственный манифест такие строки объявлял неверными.

Отдельно: индекс Lite в `main` и в опубликованном 8.4.7-L.zip указывал на ревизию гиста
от 26.07 — редакция Lite грузила модули 8.4.6. Проверка целостности молчала, потому что
обе её стороны брались из одного файла.

### Fixed
- Цена Sonnet 5 ($2/$10), Sol ($4/$0.40/$20), `deepseek-v4-flash` ($0.22/$0.66 off-peak).
- Дедлайны августа (05.08, 26.08, 31.08) переведены в исполненные — числились предстоящими.
- Контекст `grok-4.3` — 1M, `grok-4.20` — 2M, Gemini 3.1 Pro — 2M.
- 35 запрещённых алиасов `-latest` в H и N; маршруты на несуществующие `moonshot-v2-128k`
  и `moonshot-v2-8k`; legacy `gpt-5.5` как цель agentic.
- `gemini-3.8-flash` (bulk primary, GA 02.09) внесён в реестры — был объявлен, но недостижим.
- Живой слой плагина синхронизирован с чат-формой; 59 висячих ссылок; пункт `[42]`.
- Пять пунктов меню H, выводившихся без модулей; подписи агентов `[7]` и `[9]`.
- Каталог ошибок объявлен как G1–G22; семь имён G-кодов сведены к одной форме; G3 описан в C.
- Формула G10: cached input дорожает вдвое вместе с обычным.

### Changed
- Таблицы «задача → модель» пересобраны по срезу 04.09 — прежний блок был датирован 02.05.
- Счётчики дедлайнов пересчитаны от 10.09.
- Штампы свежести живого слоя: 2026-09-04 стоял на одном файле из двенадцати.

### Tooling
- `tools/verify_gist_live.py` — сверка индекса Lite с живым содержимым гиста. Обе стороны
  сверки берутся из разных источников: файл качается по адресу без вшитой ревизии.
- `tools/build_lite_chunks.py` — пересборка разрезанного чанка теряла границы секций
  и удваивала vendors-модули, дублируя их содержимое между собой.

## [8.4.7] — 2026-09-05 · Свежие live specs, оптическое сжатие снято, статусы моделей выправлены

> ✅ **Выпущено 2026-09-05**, тег `v8.4.7`. Индекс Lite в этом выпуске указывал на ревизию гиста от 26.07 — исправлено в 8.4.8.
