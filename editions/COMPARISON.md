...
> Все 4 редакции — **одна архитектура, разные хосты и форм-факторы**. Не уверены — берите ту, что заточена под вашу основную модель.
...
- **Источник.** Выделенный **unpinned (latest) Gist** со статичным именем файла `live_specs.md` — при апдейте меняется только содержимое (`VERSION: 8.5 → 8.6…`), ссылка не «протухает». Автор обновляет файл одним действием, без браузера и 2FA.
...
- Источник механики: `editions/8.4.3-L/CHANGELOG.md` (8L.3); механика переносится в 8C/8H/8N.
