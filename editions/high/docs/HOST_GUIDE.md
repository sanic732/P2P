...
> 🆕 **Нативный автодетект хоста (8.4.1):** `HOST_MODEL` теперь по умолчанию **пустой** — прелоадер
> сам определяет хост (SELF_IDENTIFY) на старте. Ставить вручную нужно, только если
> автодетект не уверен (тогда покажется список `[1..10]`) или чтобы форсировать хост.
> Инструкции «Установи: `HOST_MODEL: "…"`» ниже — для ручного/форс-режима.
...
```python
model="deepseek-v4-pro"      # НЕ deepseek-chat (G16, retire 2026-07-24)
...
MIT лицензия. ## секции. Строгий лимит контекста.

```python
model="glm-5.1-flash"
temperature=0               # для JSON output
# HARD LIMIT: 100K tokens (G19)
```
