...
// ═══════════════════════════════════════════════════════
// P2P v8H.3 — SECURITY MODULE (!security.md)
...
// Использует SIR Scanner (!!core_v8H §3) для homoglyph/zero-width/encoding детекции.
...
Интеграция: SIR Scanner (homoglyph/zero-width/encoding) из !!core_v8H §3.
```
...
Когда: высокий stakes (медицина/право/финансы/факты). Связь: !metrics [v8H.3] Quality Eval.
```
...
Аддитивный модуль (не конфликтует с базой v8H). MUTEX: требует GUARDIAN:ON — если GUARDIAN:OFF
при активном !security → CONFLICT_RESOLVER принудительно включает GUARDIAN на время аудита.

VERSION_METADATA:
  SYSTEM:      P2P v8H.3 Normal · Security Module
  TECHNIQUES:  Injection_Detection, Jailbreak_Classification, Prompt_Hardening, SelfCheckGPT
  SOURCE:      donor v8C.3 !security.md, универсализирован (host-gated делимитеры, SIR-интеграция)
  MENU_ITEM:   30
  COMPATIBLE:  !!core_v8H.md | !!db_v8H.md | !metrics.md
