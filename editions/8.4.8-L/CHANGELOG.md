...
Выпуск про данные: снимок и профили догнали срез 04.09.

### Fixed
- **`claude-opus-5` вернулся в снимок API-строк** — был объявлен основным, но в самом
  снимке отсутствовал; `claude-fable-5-1` не значился в профиле Claude, хотя стоит
  приоритетом 1 в матрице возможностей.
- **`claude-opus-4-6` перестал дублироваться** в списке API-строк (три файла).
- **`gemini-3.8-flash` добавлен в профиль Gemini** как bulk-primary (GA 02.09).
- **Дедлайны 19.07 и 24.07 помечены исполненными** — стояли `[ACTIVE]`.
- **Предел swarm — 300 агентов** (порог 40 из G20 остаётся: выше него нужен async).

- **Цена Sonnet 5 — $2/$10.** Строки «$3/$15 с 01.09» убраны: подорожание отменено 10.08,
  а дата уже наступила, и сборка называла вдвое завышенную цену на модель по умолчанию Free/Pro.
- **`claude-opus-4-1-20250805` больше не значится предстоящим ретайром** — модель снята 05.08.
  Вместе с ней в прошедшее время переведены дедлайны 26.08 (OpenAI Assistants API)
  и 31.08 (`kimi-k2.5`, часть `moonshot-v1`).
- **Цена GPT-5.6 Sol — $4 / $0.40 cached / $20** (промо «не раньше» 21.11), выше 272K — $8 / $30.
  Прежние $5/$30 и $10/$45 остались от старого прайса.
- **Цена `deepseek-v4-flash` — $0.22/$0.66 off-peak, $0.44/$1.32 peak.** До этого сборка
  называла шесть разных значений в разных файлах.
- **Контекст `grok-4.3` — 1M.** 2M только у `grok-4.20` (Heavy-16). Заодно выправлен
  Gemini 3.1 Pro: 2M, а не 1M.
- **G10 (порог 272K): кэш обрыв не переживает** — cached input дорожает вдвое вместе
  с обычным. Прежняя пометка «cached EXEMPT» и вывод «стабильный префикс спасает» сняты.
- **Каталог ошибок объявлен как G1–G22**, а не G1–G20: коды G21 и G22 в нём давно есть.

### Changed
- Рекомендации по задачам: код — Opus 5, агенты — Opus 5, creative — Fable 5 и GPT-5.6 Terra
  вместо legacy GPT-5.5 и Opus 4.7.
- Модули на гисте пересобраны из источников 8.4.8-H.

## [8.4.7-L] — 2026-09-05

### Fixed
- **`claude-sonnet-4-6` больше не помечен снятым** — модель активна, снятие не раньше
  17.02.2027; 30.06 сменился только дефолт.
- **Цена Sonnet 5 — $2/$10**: подорожание до $3/$15 с 01.09 отменено 10.08.

> Live specs Lite тянет через fetch из гиста — обновляются публикацией гиста, не сборкой.

## [8.4.6-L] — 2026-07-26

Изменения с версии 8.4.5-L. В Lite действует жёсткая экономия контекста, поэтому из обновления
данных перенесено только то, что влияет на выбор модели; арена, наблюдения сообщества и
пояснительный текст остались во внешнем слое.

### Added
- В профиль Claude: **`claude-opus-5`** как основная модель (thinking включён по умолчанию),
  `claude-sonnet-5`, `claude-opus-4-8` (доступен только через API), `claude-haiku-4-5`.
  Отмечены снятие `claude-opus-4-1` 05.08 и переход Fable 5 на usage credits.
- В профиль Gemini: **`gemini-3.6-flash`** (рабочая лошадка) и **`gemini-3.5-flash-lite`**
  (самый дешёвый уровень, основной для Lite).
- **Две новые записи в базе ошибок:** G21 (несовпадение заявленной и фактической модели) и
  G22 (агентная опасность GPT-5.6 Sol) — обе в быстрый справочник: G22 как критическая,
  G21 как высокая.
- `kimi-k3` в реестр моделей с меткой **ACCESS-RISK — не основной**; `qwen3.7-plus`;
  пометка «`qwen3.8-max-preview` не маршрутизировать».

### Changed
- **Версия отображается одна во всех местах** — `8.4.6-L`, включая вопрос о выборе хоста
  при старте и заголовок `INSTALL.md`.
- **BOOT стал легче на ~750 символов (≈250 токенов).** Для Lite это заметнее, чем для
  остальных сборок: убраны служебные хвосты (даты правок, построчные changelog-заметки,
  метки поколений) — история живёт в этом файле, а не в загружаемом ядре.
- **Номер версии теперь стоит в одном месте каждого файла — YAML-шапке**, а не в заголовке,
  поле `scope` и хвостовом блоке одновременно.
- Пункт меню `[32]` называется «Что нового в этой версии» — раньше в названии стоял номер,
  который надо было править при каждом выпуске.

### Fixed
- **G16 — правило миграции DeepSeek было перевёрнуто:** бывший `deepseek-reasoner` идёт на
  **v4-pro**, а не на v4-flash-thinking. Алиасы мертвы с 24.07 (ответ 404 либо 400 — считать
  оба), линейка V4 официально в статусе PREVIEW.
- **G14 — цена Grok на кэшированный ввод:** `$0.30` short / `$0.60` long вместо
  унаследованной `$0.50`; порог 200K удваивает тариф **вместе с кэшем**. Моделей
  `grok-4.5-heavy` / `-expert` / `-fast` не существует.
- **G6 — токенизатор:** канон **~+30%** (одна официальная цифра вместо вилки) +
  Token Counting API.
- **G10 — длинный контекст:** ×2 на некэшированный ввод, ×1.5 на вывод, **кэшированный ввод
  не дорожает**. Длинный контекст Terra/Luna помечен как недокументированный.
- **G13 — на `gemini-3.6-flash` баг не тестировался:** модель не проверена, а не исправлена;
  обходные приёмы применять и к ней.
- **Grok в EU:** доступ открыт 21.07, но без гарантии размещения данных — ограничение
  переформулировано с недоступности на персональные данные.

### Внешний слой (gist) — обновлён вместе с выпуском
Арсенал в gist пересобран из актуальных файлов High: прежние чанки собирались 27.06-13.07,
внутри стояла версия `v8H.3`, не было `claude-opus-5` и записей G21/G22, а про снятые
алиасы DeepSeek говорилось в будущем времени.
...
- Ссылки вида `!!core_v8H §7` переведены на имена блоков Lite: нумерованных секций
  в ядре L нет, и прямая замена дала бы ссылку в никуда.
- Доставка проверена скачиванием обратно: 14 из 14 по контрольной сумме и маркеру конца файла.
- Канарейка намеренно осталась на замороженном пине (`CANARY_POLICY: frozen_probe`) —
  она проверяет способность хоста к fetch, а не актуальность контента.
...
## [8.4.5-L · 2026-07-25 · Code] Фикс связности + EXP-D + верификатор Lite

### Канон (одна строка)

- **`!!core_v8L.md` → `AGENT_WEIGHTS`:** ссылка `!!db_v8L.md §DYNAMIC_WEIGHTING` вела в пустоту —
  секции с таким именем в `!!db_v8L.md` нет и не было, реальная называется **`§QUORUM_WEIGHTS`**
  (строка 147). Дефект присутствовал в опубликованном релизе 8.4.5 и во всех вариантах.
  Исправлено в каноне; в экспериментальные ветки правка НЕ вносилась — она есть в обеих
  группах A/B и на сравнение не влияет, а менять CONTROL нельзя (он побайтовая копия релиза).

### Новый инструмент — `tools/verify_lite.py`

Статическая верификация boot-набора Lite: 15 проверок (C1–C15), ~270 объектов.
Манифест ↔ ядро ↔ preloader: `requires`, `mutex`, `fallback`, `COMMAND_CHUNK_MAP`,
нумерация меню и ссылки на пункты, плейсхолдеры `{VAR}` без определения, ссылки на секции
`!!db_v8L.md`, присвоение `LOAD_MODE` на всех ветках `JUDGE`, совпадение списка `seed == ALL`
с манифестом, уникальность EOF-маркеров, sha256/url, сверка локальных копий чанков.
Плюс симуляция резолвера (KB и число запросов на команду) и режим `--baseline` —
показывает **новые** ошибки относительно эталона, не подавляя унаследованные.

**Доказано, что умеет провалиться:** `--selftest` вносит 10 искусственных поломок и требует,
чтобы каждая дала **новую** ошибку сверх набора базы — 10 из 10. Сверять только exit-код
оказалось недостаточно: база уже была красной из-за `§DYNAMIC_WEIGHTING`, и `exit=1` вернулся бы
без всякой поломки. Ровно тот тихий отказ, от которого проверка должна защищать.
При нуле проверенных объектов — `FATAL`.

### EXP-A — починены 2 дефекта, внесённые самим экспериментом

- опечатка `РЕАЛьНы` → `РЕАЛЬНЫ` в `!!core_v8L.md` (в `DIFF_EXP-A.md` было написано верно —
  поставка разошлась с документом);
- `LOAD_MODE` не присваивался на ветках `FETCH_PARTIAL` / `FETCH_FAILED_WITH_STATUS`,
  хотя баннер его печатает. Теперь присваивается при любом исходе, неуспех выражается статусом.

Вариант готов к живому прогону.

### EXP-D — subchunk routing (новый эксперимент)

`CORE_PLUS` разрезан скриптом по **уже существующему в файле шву** (`!agents.md` |
`!pipeline.md`) на `CORE_PLUS_AGENTS` (6.6 KB) и `CORE_PLUS_PIPELINE` (15.6 KB);
содержимое не переписывалось. `requires` у RAG / REASONING / OPTIMIZATION приведены
к фактической потребности, проверенной по телу каждого чанка (их `depends_on` унаследован
из редакции 8H и описывал происхождение, а не рантайм-нужду).

| Команда | было | стало | Δ | запросов |
|---|---|---|---|---|
| `/p2p-rag` | 66.5 KB | 5.4 KB | −92% | 3 → 1 |
| `/p2p-reasoning` | 27.7 KB | 12.3 KB | −56% | 2 → 2 |
| `/p2p-optimize` | 34.4 KB | 19.0 KB | −45% | 3 → 3 |

Число сетевых запросов не выросло ни на одной команде — принципиальное отличие от
микроядерной схемы, где дробление умножает round-trip'ы и усиливает главный отказ Lite.
`url` подчанков = `PENDING_GIST`; введён `fallback: PARENT(CORE_PLUS)` с определением
в новом блоке `FALLBACK_SEMANTICS`, поэтому вариант работоспособен и до заливки в гист —
деградирует ровно к текущему поведению.

**Прогоны:** CONTROL, EXP-A, EXP-D, канон — все проходят; единственная ошибка вариантов —
унаследованный `§DYNAMIC_WEIGHTING`, в каноне уже исправлен.

## [experiments · 2026-07-20 · Code] A/B-эксперимент: EXP-A + EXP-B

Собраны три раздельные сборки в `experiments/` (`.gitignore`, не коммитятся в `main`).
Цель: A/B-тест двух независимых гипотез против контрольной группы.

### Собрано

- **`CONTROL/`** — точная копия `editions/8.4.5-L/boot/`, изменений ноль.
  Скрипт связности (B3): ALL CHECKS PASSED (8+10+1+1+3+5 = все OK).

- **`EXP-A/`** — BOOT_MINIMAL: переименование состояний, lifecycle, capability probe.
  Изменены: `_preloader_v8L.md` (A1/A2), `!!core_v8L.md` (A3/A4).
  Не тронуты: `_index_v8L.md`, `!!db_v8L.md`, гисты, sha256, меню.
  Одна переменная: `БЕЗОТКАЗНОСТЬ` → `TOOL_CAPABILITY_GROUNDING` (две остальные составляющие не тронуты).

- **`EXP-B/`** — доказуемая верификация: арифметическая канарейка, цветовой отпечаток, связность.
  Изменены: `_preloader_v8L.md` (B1/B4), `_index_v8L.md` (B3/B4), `!!core_v8L.md` (XML_POLICY/SELFCHECK).
  Новые файлы: `gist_checksums.md` (файл 13), `BB_COLOR_MAP.md` (BB-код для 4PDA).
  Не тронуты: продакшн-гисты (`7727406…`, `a64245c3…`), `!!db_v8L.md`, меню.
...
- `experiments/CONTROL/_ПРОТОКОЛ_ТЕСТА.md`
- `experiments/EXP-A/_ПРОТОКОЛ_ТЕСТА.md` + `DIFF_EXP-A.md`
- `experiments/EXP-B/_ПРОТОКОЛ_ТЕСТА.md` + `DIFF_EXP-B.md` + `BB_COLOR_MAP.md` + `gist_checksums.md`

---

## [8.4.5-L hotfix · 2026-07-19b · Code] Консистентность манифеста и флагов

Найдено внешним статическим аудитом (Copilot 365 / GPT-5.5 thinking) поверх релиза 8.4.5,
перепроверено по файлам. Только Lite — остальные сборки не затронуты.

### Fixed
- **🔴 `/p2p-download` грузил 4 чанка из 12.** `_index_v8L` после пересборки содержал
  11 чанков + LIVE, а `!!core_v8L` разворачивал `seed == ALL` в статический список
  `[CORE_PLUS, SESSION_CORE, SESSION_METRICS, LIVE]`. `HOST_ENGINE`, `VENDORS`, `RAG`,
  `REASONING`, `ROUTE`, `COMPRESS`, `SECURITY`, `OPTIMIZATION` молча не догружались.
  Теперь `ALL` = все записи `GIST_ROUTING_TABLE`; манифест объявлен единственным
  источником истины, статический список убран.
- **🔴 `/p2p-security` был нерабочим в чате.** `ENV_CHAT_GENERIC` и `ENV_GEMINI_STUDIO`
  ставят `GUARDIAN = OFF`, при этом чанк `SECURITY` требует `GUARDIAN:ON` и имеет
  `mutex: [GUARDIAN_ON]` — то есть в самой массовой среде пункт `[41]` упирался в мьютекс
  с неопределённым поведением. Введён `GUARDIAN_AUTO_ELEVATE`: команда поднимает GUARDIAN
  на время выполнения, сообщает об этом строкой и возвращает прежнее значение.
- **🟡 `PREFETCH_STEP` не срабатывал никогда.** Условие `WHERE value in [true, or]` не
  совпадало ни с одним фактическим значением `VERSION_COMPAT` (в дефолтах — `on` и `auto`).
  Условие расширено до `[true, on, auto, or]`, допустимые значения флага задокументированы.
  _(Внешний аудит счёл это опечаткой в слове `or`; на деле `or` — легитимное значение,
  проблема была в рассогласовании условия с набором значений.)_

### Clarified
- **`CANARY_POLICY: frozen_probe`** — зафиксировано явно, что канарейка намеренно запинена
  на отдельную ревизию `gist_route.md` и не обязана совпадать с ревизией записи `ROUTE`.
  Канарейка проверяет способность хоста к fetch, а не актуальность контента. Расхождение
  трёх пинов (preloader / FETCH_CANARY / ROUTE) — ожидаемо, не баг.
- **Граница жизненного цикла LIVE.** `/host`, `/lang`, `/p2p`, `/start` помечены «нет fetch» —
  уточнено, что это относится к самим командам: они рендерят уже инициализированное
  состояние, а единственная загрузка LIVE выполняется один раз в `_preloader ON_LOAD`
  до первого показа меню.

### Отложено (не в этот хотфикс)
- Переименование `CORE_ONLY` → `BOOT_MINIMAL` + lifecycle-состояния (`BOOT_READY` /
  `CHUNK_READY` / `UNRESOLVED`) и `Capability Probe` со статусами `FETCH_OK` /
  `FETCH_PARTIAL` / `FETCH_FAILED_WITH_STATUS` — требуют прогона по хостам, идут в 8.4.6.
- Дробление чанков на подчанки — только после того, как манифест начнёт генерироваться
  скриптом: ручная синхронизация уже дала три ошибки связности за один день.
