...
LITE_SNAPSHOT:
  LAST_VERIFIED: 2026-09-10        // единое имя поля во всех редакциях (было AS_OF)
  DEADLINES:
    ...
    [DEADLINE 2026-09-14 T-4] Claude Code: постоянные +25% (сам вендор называет это −17% против сегодня)
      [S] источник — запись 29.08 в соцсети; статья help-центра говорит «вернутся к стандартным», тело не читалось
...
KNOWLEDGE_LAYERS:
  ...
  DYNAMIC:   #LINK_CLAUDE → gist_vendors_claude · #LINK_GPT/#LINK_GEMINI → gist_vendors_frontier ·
             #LINK_GROK → gist_vendor_grok · #LINK_DEEPSEEK/QWEN/KIMI/GLM → gist_vendors_budget.
             #LINK_LIVE → gist_live.
             PRIORITY: gist_live > vendor-чанк > db defaults.
...
TECHNIQUES:
  ...
  COMBINATOR:
    ...
    reasoning_model + BRUTAL_EDITOR → DOWNGRADE. VS + GASLIGHT_SAFE → RETAIN GASLIGHT_SAFE. POSITIVE_FRAMING не к hard-safety.
...
TECHNIQUE_COMBINATOR:
  DO_NOT_BLOCK:
    ...
    VERBALIZED_SAMPLING ≠ USC ; GEPA ≠ GoT ; MASPO ≠ ToT
  RULE: ANON/VECTOR ОБЯЗАН свериться с этим блоком до VETO любой reasoning/rag техники.
  NOTE: GEPA/MASPO/SePO — фреймворки-процессы (нужен eval-harness) → в Lite справочно, не активируются.
...
G_ERRORS_QUICK_REF:
  ...
  HIGH:     G3, G9, G10, G11, G13, G20, G21
  MEDIUM:   G4, G6, G8, G12, G15, G17, G18

API_STRINGS:
  CLAUDE:
    claude-opus-5              // PRIMARY, GA 24.07, thinking on by default
    claude-fable-5             // ⚠ COST-GATED с 20.07 (usage credits) — только явный вызов
    claude-sonnet-5
    claude-opus-4-8            // ACTIVE, НЕ депрекирован; API-only surface
    claude-opus-4-7
    claude-opus-4-6
