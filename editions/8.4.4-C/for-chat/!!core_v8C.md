...
**Type Q — Lossy Optical Misfire:** L-OPTICAL/pxpipe applied to byte-exact content (code/JSON/hashes/keys) OR on a non-reader model (Opus/Sonnet confuse ~7%, verbatim hex 0/15). Mitigation: PXPIPE_GATE — reader ∈ {Fable 5, GPT-5.6} + byte-guard → text-sidecar.
