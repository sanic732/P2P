# P2P v8N.3 — FAQ и Ошибки

---

## Частые вопросы

**Q: В меню пункты 26-31 (RAG, Reasoning, Routing, Compression, Security, Optimization) показаны как 🔒. Почему?**
A: Это НЕ ошибка — так и задумано (v8.4.1). Пункты [26-31] — опциональные модули; их файлы
(`!rag.md`, `!reasoning.md` …) грузятся отдельно, чтобы экономить токены. Пункт становится рабочим
ТОЛЬКО когда его файл реально приложен в контекст. Приложил `!rag.md` рядом с base → на ближайшем
`/start` (или по триггеру «rag/retrieval…») пункт [26] разблокируется. Не приложил → он в 🔒-футере.
Флаг `MODULE_RAG=true` сам по себе пункт НЕ открывает — нужен именно файл (иначе нечего запускать).

**Q: Меню не показывает модуль, хотя я приложил его файл.**
A: Проверь, что приложен именно файл-модуль целиком (с заголовком `— … MODULE (!x.md)` и frontmatter
`id:`/`menu_item:`), а не просто упоминание. Детект (`!!core_v8N.md §4 EXTENSIONS_SCAN`) смотрит на тело
файла. Затем вызови `/start` или `/menu` — скан прогоняется заново и пункт появится.

**Q: На Qwen хост не определился автоматически, спрашивает списком — это баг?**
A: Нет. У Qwen самоопределение слабое, поэтому штатно показывается список `[1..10]` — выбери `[6] qwen`.
Меню появится сразу после выбора. Подробнее — `HOST_GUIDE.md` → «Определение хоста».
...
A: `/p2p-deadline` или пункт 24 меню. Проверит все legacy strings и выдаст замены.
Главные: `claude-* dated legacy aliases (RETIRED 2026-06-15)` → актуальные (дедлайн 2026-06-15),
`deepseek-chat` → `deepseek-v4-pro` (дедлайн 2026-07-24).
...
**Q: P2P v8N.1 vs v8C.1 — что выбрать?**
...
v8N.1 (Normal Edition) — если работаешь с несколькими моделями или хочешь портативную систему (23 файла, 8 хостов, Translation Layer).
