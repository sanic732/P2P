...
G10 — точная механика: выше 272K весь запрос считается по **×2 input, ×2 cached input**
  ...
  была ошибкой и исправлена в 8.4.7: кэш от подорожания за порогом НЕ спасает. Порог у xAI устроен иначе (см. tier3) — одна общая заглушка два случая не описывает.
