...
# P2P — SESSION METRICS v0.2 (!metrics.md)
...
metrics_v02:
  ...
  chain_runs: 0           # Сколько раз запускался Chain Mode (!!db_v8C.md chain_orchestrator)
  ...
  # Errors caught (types A-P from !!db_v8C.md + G-errors G1-G20)
...
> DEPENDS_ON: !memory.md, !scope.md

### Механизм

Routing Memory работает ТОЛЬКО при наличии memory_block от предыдущей сессии
(через !memory.md CAPSULE или вставленный вручную).
...
При наличии <session_metrics> в memory_block:
  ...
  IF main_correction_pattern указан:
    → При обнаружении похожего паттерна в текущем запросе:
      сразу применить fix из !debug.md, не ждать "не то"
...
Projects:    memory_block в Project Knowledge через !memory.md экспорт
