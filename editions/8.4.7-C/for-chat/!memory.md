...
tags: memory, capsule, cross-session, context-compression, state-persistence, on-demand, context-engineering
triggers: "память", "CAPSULE", "сохрани контекст", "восстанови", "/p2p-capsule", "[24]"
---

# P2P — MEMORY BRIDGE (!memory.md)
...
> Мета-фрейминг: `!memory.md` / CAPSULE = приём **note-taking** в CONTEXT ENGINEERING (`!compression.md`, раздел «CONTEXT ENGINEERING (Anthropic framing)»).
...
ACTIONS:
  1. SCAN текущей сессии для:
     ...
     - Session metrics из !metrics.md → <session_metrics>
  ...
  4. Обновить !sandbox.md SESSION_GOAL с NEXT_ACTION
```
...
ON NEW SESSION:
  1. CHECK memory sources (priority):
     ...
     c. !sandbox.md SESSION_GOAL field → lightweight
