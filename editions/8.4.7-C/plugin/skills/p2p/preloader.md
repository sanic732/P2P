...
module_type: base
depends_on: none
scope: P2P entry point — USER_CONTEXT detection, PROJECT_CARD, TRI_MODE_BRIDGE v3 environment detection, load order declaration. Always loaded first.
...
`[P2P 8.4.7-C | Среда: {СРЕДА} | Guardian: {ON/OFF}]`
...
ПОРЯДОК ЗАГРУЗКИ (ON-DEMAND — по триггеру):
  ...
  contract_builder.md     ← Contract Builder, Translation Layer
  debug_engine.md        ← Debug Engine
  domain_knowledge.md       ← Domain Knowledge
  exploration_mode.md  ← Exploration Mode
  memory_bridge.md       ← Memory Bridge, CAPSULE
  mentor_method.md       ← Mentor Method
  session_metrics.md      ← Session Metrics
  scope_helm.md        ← SCOPE.HELM
  templates_library.md    ← Template Library (детальная)
...
4. Если запрос = "СТАРТ" / "/p2p" БЕЗ АРГУМЕНТОВ / "/menu" / "[31]" / "full ui menu" → показать ПОЛНОЕ меню одним экраном: лого + арт-баннеры (если art.md загружен) + строка РЕЖИМОВ (буквы C/A/M/S/Q/H/E) + все пункты [1-42] (см. core.md).
   ⚠ Если запрос = "/p2p <задача>" (непустой аргумент, не `start`/`menu`) → меню НЕ показывать.
   Маршрут: SIR Scanner → Tier → Contract Builder; вывод в синтаксисе TARGET_MODEL (P1 CROSS_MODEL_GENERATION_AWARENESS).
5. Если запрос = задача (без команды) → НЕ «сразу начинать» мимо системы:
   тот же маршрут, что в п.4 — SIR Scanner → Tier → далее по таблице ROUTE (core.md).
   Tier ≥ T2 → предложить QUORUM в порядке, заданном PILOT_MODE.
   ⚠ Формулировка «сразу начинать» удалена 2026-07-20: она описывала обход системы —
   ровно тот путь, которым задача уходила в обычную генерацию (инцидент 19.07).
6. Выведи: [P2P 8.4.7-C | {СРЕДА} | {TIER}]
```
