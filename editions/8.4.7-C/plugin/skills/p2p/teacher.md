...
> Источник истины для всех учебных сценариев. Не использовать как замену `core.md`.
...
- FAQ покрывает 80% типичных вопросов; для остального — fallback в `core.md` + `db.md`
...
**Упражнение:**
Открой главный файл `core.md` и пролистай меню (42 пункта). Напиши какой пункт привлёк внимание.
...
```
User: /p2p
P2P: [P2P 8.4.7-C | Среда: COWORK | Guardian: ON]
     1) QUICK GEN ...
     2) DEEP DIVE ...
     ...
...
**Упражнение:**
Запусти `/p2p` (если в Code/Cowork) ИЛИ открой `core.md` и найди где описано меню. Какая среда у тебя сейчас?
...
Также T-routes (T1-T5 из exploration_mode.md) могут эскалировать.
...
A: Да через Translation Layer (`contract_builder.md`). Native — Claude. Для других моделей есть adaptation (`vendors/tier1-4.md`).
...
A: Stage 1 routing — анализирует запрос за <100 токенов, выбирает route без загрузки больших файлов. См. `intent_engine.md`.
...
A: Karpathy Coding Mode template. PRE_CODE_DECLARATION + composability M+R / M+I / M+T. См. `templates_library.md`.

### Q17. Можно ли передать P2P свою память (long-term)?
A: Да через `memory_bridge.md` Bridge protocol. Поддерживает CAPSULE injection + selective recall.
...
A: P2P запоминает какие агенты успешны на каких задачах, биасит будущие routing decisions. См. `session_metrics.md`.
