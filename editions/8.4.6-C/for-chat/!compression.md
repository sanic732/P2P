...
tags: compression, llmlingua, gist-tokens, context, kv-cache, on-demand, v8c3, context-engineering
...
## L-OPTICAL (pxpipe) — только Code/Cowork-редакция

В plugin-форме (Claude Code / Cowork) модуль compression дополнен четвёртой техникой —
**L-OPTICAL**: оптическое сжатие нарратива в PNG (pixel-space, DeepSeek-OCR arXiv 2510.18234;
исполнитель — skill `pxpipe`). Замерено ~82% экономии токенов на блок; reader-gate: Fable 5.
В chat/projects-форме техника НЕДОСТУПНА (нет исполнителя) — используй LLMLingua/Gist/CAPSULE выше.
...
techniques: [LLMLingua, Gist_Tokens, Verbatim_Deletion, Selective_Summarization, L-OPTICAL_pxpipe_code-only]
menu_item: 38
