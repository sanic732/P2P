...
# P2P — EXPLORATION MODE (!exploration.md)
...
Подход A — [название]
Агент: [TECTON|IRIS|...] | Шаблон: [из !templates.md] | Сложность: [Tier 0-4]
...
**STEP 3 — После выбора:**
- Зафиксировать выбор как routing decision (для !metrics.md)
- Выполнить через выбранный маршрут
- В конце спросить: "Этот подход сработал? [да/нет/частично]"

**STEP 4 — Если "нет" / "частично":**
- Активировать Feedback Loop (!!db_v8C.md Раздел 11)
- Зафиксировать в !metrics.md: corrections_requested++
...
```
ROUTING:   !!core_v8C.md → SIR Scanner → fallback hook при confidence < 0.55
METRICS:   !metrics.md → exploration_triggers++
FEEDBACK:  !!db_v8C.md Раздел 11 → автозапуск при "нет"/"частично"
AGENTS:    !agents.md → AXIOM оценивает варианты
COMMAND:   /p2p-explore
MENU:      item 22 в !!core_v8C.md
```
