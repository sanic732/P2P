...
//   1) READER GATE  — only image for models proven to read renders (Fable 5 only).
...
// Fable-only. `gpt-5.6` был в списке до 2026-07-26 и убран вслед за upstream:
// автор pxpipe вывел `gpt-5.6-sol` из дефолтного скоупа после raw-recall пилота
// (0/4 точных идентификаторов, четыре ВЫДУМАННЫХ значения — на обоих профилях,
// 6×11 и 5×8). Для P2P это прямое столкновение с запретом фабрикации: сжимать
// нарратив для читателя, который выдумывает идентификаторы, нельзя.
// Обойти осознанно: `--force`. Вернуть — только после собственного замера recall.
const SUPPORTED_READERS = new Set(['claude-fable-5', 'fable-5', 'fable']);
