...
version: 8.4.6-C
module_type: meta
scope: P2P master index — all files, load order, tags, module types.
...
│   ├── contract_builder.md           Contract Builder, Translation Layer полный
│   ├── debug_engine.md              Debug Engine, G-error diagnosis
│   ├── domain_knowledge.md             Domain Knowledge + React 19 / Kotlin / KMP reference (merged)
│   ├── exploration_mode.md        Exploration Mode (Cortex Patch A)
│   ├── intent_engine.md             Intent Engine — 9D, 36 anti-patterns, 12 tool routes (port v7C.2)
│   ├── memory_bridge.md             Memory Bridge, CAPSULE protocol
│   ├── mentor_method.md             Mentor Method, Socratic pattern
│   ├── session_metrics.md            Session Metrics v0.2, Routing Memory
│   ├── scope_helm.md              SCOPE.HELM v1.2 — SPLITTER/CAPSULE/ROUTER
│   ├── templates_library.md          Template Library extended A-M
│   ├── tool_budget.md        Tool Budget, re-injection protocol
│   ├── user_context.md       User Context extended, adaptive behavior
│   ├── visual_suite.md             Visual/Video/Audio Suite (port from v7C.2)
│   ├── writing_suite.md            Writing Quality Control (port from v7C.2)
│   ├── sandbox_user.md            User Editable Sandbox (port from v7C.2)
│   └── teacher.md            [v8C.2 NEW] Curriculum 5 уровней + Q&A + cheatsheet
│
├── ON-DEMAND v8C.3 (по VERSION_COMPAT, меню [35-40]):
│   ├── rag.md                 RAG / RAPTOR / векторный поиск
│   ├── reasoning.md           CoT / Self-Consistency / MCTS / TTS
│   ├── routing.md             Выбор модели + effort (advisor)
│   ├── compression.md         LLMLingua / Gist Tokens
│   ├── security.md            Аудит промптов / injection defense
│   ├── optimization.md        APO / OPRO / автооптимизация
│   └── art.md                 OPTIONAL ASCII-баннеры режимов (eye-candy)
│
...
| `react` | domain_knowledge.md |
| `kotlin` | domain_knowledge.md |
| `intent` | intent_engine.md |
| `anti-patterns` | intent_engine.md, core.md |
| `tool-routing` | intent_engine.md |
| `error-taxonomy` | debug_engine.md |
| `tone-spectrum` | writing_suite.md |
| `ui-replication` | visual_suite.md |
| `failure-modes` | agents.md |
| `format-enforcement` | contract_builder.md |
| `classic-frameworks` | templates_library.md |
