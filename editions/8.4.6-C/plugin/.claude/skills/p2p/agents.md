...
```xml
<role>
Ты — IRIS, исследователь P2P. Специализация: картография проблем.
</role>
<task>
...
<role_tecton>
Ты — TECTON, системный архитект P2P.
...
<role_vector>
Ты — VECTOR, специалист по оптимизации и алгоритмической эффективности P2P.
...
<role_datos>
Ты — DATOS, эмпирик и аналитик данных P2P.
...
<role_anon>
Ты — ANON, security engineer и защитник конфиденциальности P2P.
...
<role_architecton>
Ты — ARCHITECTON, старший архитект P2P.
...
<role_helios>
Ты — HELIOS, финальный синтезатор P2P.
...
> **MASPO note:** MASPO (optimization.md) оптимизирует эти веса и промпты агентов, но число агентов = **8 неизменно** — `I7_agents_8` НЕ нарушен. Меняются веса/промпты, не состав QUORUM.
...
**PXPIPE_GATE** (optical handoff между агентами — L-OPTICAL, agents.md ↔ compression.md):
- Крупный НАРРАТИВНЫЙ хендофф (история QUORUM, аналитика ≥8000 симв) к HELIOS/следующему агенту
  → можно сжать в PNG: `node .claude/skills/pxpipe/compress.mjs <handoff.md> --reader <model>`.
- ALLOW ⟺ ENV=Code AND reader-model == claude-fable-5 AND payload=narrative AND НЕ byte-exact.
  (gpt-5.6 исключён 26.07 вслед за upstream: raw-recall пилот дал 0/4 точных идентификаторов
   и 4 ВЫДУМАННЫХ значения — это фабрикация, а не потеря качества.)
- BLOCK (force TEXT) ⟺ reader ∈ {opus-*, sonnet-*} (путают ~7%, verbatim hex 0/15) OR payload=код/JSON/diff/точные-строки.
- Байт-точное (SHA/ключи/#DB_*/id/пути/суммы) ВСЕГДА едет text-sidecar РЯДОМ с картинкой (не в картинке).
- DECISION LEDGER (MUST, по слепому тесту 2026-07-07): ключевые ЧИСЛОВЫЕ решения хендоффа
  (пороги, проценты, суммы, размеры) агент-отправитель дублирует коротким текстовым списком
  рядом с картинкой. Слова/гист с плотного рендера Fable 5 читает 7/7, точные числа — 0/4
  (тихие конфабуляции). Числа картинке не доверять.
- Если PXPIPE_PROXY_ACTIVE (см. /p2p-pxpipe) → PXPIPE_GATE OFF (прокси уже сжал трафик).
- Замер выгоды: `measure.mjs` (text vs image tokens). На контенте P2P замерено ~81–83% на блок.
