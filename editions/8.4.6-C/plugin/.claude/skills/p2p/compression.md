...
### Optical Compression (L-OPTICAL) — pixel-space (новое в v8C.3)
**Источник:** DeepSeek-OCR «Contexts Optical Compression» (arXiv 2510.18234) + pxpipe (teamchong, MIT).
**Суть:** рендер объёмного НАРРАТИВА в плотный PNG. Vision-модель тарифицирует картинку
по площади пикселей, а не по числу символов → на реальном контенте P2P замерено **~81–83%
экономии токенов** (ratio ~5×, одна страница). Это ДРУГАЯ ось, чем LLMLingua (token-space)
и Gist (learned marker) — см. CONFLICT_RESOLVER + `db.md` disambiguation.

```
[L-OPTICAL]
Исполнитель: skill `pxpipe` → node .claude/skills/pxpipe/compress.mjs <file> --reader <model>
Гейты (ЖЁСТКИЕ, enforcement в compress.mjs):
  READER : model == claude-fable-5                   иначе → passthrough TEXT
           (gpt-5.6 убран 26.07 вслед за upstream: raw-recall пилот дал 0/4 точных
            идентификаторов и 4 выдуманных значения — фабрикация, а не экономия)
  PROFIT : narrative_chars ≥ 8000 (деф.), ratio<10×   (режим <10× ≈ 97% точности, DeepSeek-OCR)
  BYTE   : SHA/ключи/#DB_*/id/пути/model-strings → text-sidecar РЯДОМ с картинкой (verbatim-safe)
Что МОЖНО: аналитика, логи рассуждений, история QUORUM, словесная архитектура.
Что НЕЛЬЗЯ имиджировать: код, JSON, diff, точные строки → остаются текстом/в sidecar.
Взаимоисключение: если активен прокси-режим (PXPIPE_PROXY_ACTIVE, см. /p2p-pxpipe) →
  L-OPTICAL OFF (прокси уже жмёт весь трафик на проводе, двойной имиджинг не нужен).
```
...
IF ctx_usage 80-90%:  → Verbatim Deletion + LLMLingua на старых exchanges
        └ ЕСЛИ ENV=Code AND model∈readers AND есть narrative-блок ≥8000 и НЕ byte-exact:
             → L-OPTICAL (pxpipe) вместо/вместе с L4 на этом блоке
...
| **L-OPTICAL (pxpipe)** | **pixel-space (рендер в PNG)** | lossy, reader-gated | нарратив ≥8000, model=Fable 5 |

ANON MUST NOT блокировать L-OPTICAL как fabricated: это отдельная техника (DeepSeek-OCR),
