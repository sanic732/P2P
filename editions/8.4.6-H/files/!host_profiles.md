...
// ═══════════════════════════════════════════════════════
// P2P — HOST PROFILES (the host-choice brain)
...
GROK_ADVANTAGE_RULE:  // явное требование ТЗ
  WHEN HOST_MODEL == grok:
    ...
    → IF GROK_PACK_OFFER==on (_preloader GROK_FLAGS) → однократно предложить сборку нативного
      Grok Heavy-16 пака + строгого JSON (!grok_heavy.md GROK_HANDSHAKE.TRIGGER_1). Не молча.
  ELSE (любой другой хост):
    → !agents.md использует СИМУЛИРОВАННЫЙ QUORUM (8A.1, последовательные раунды 1-8)
    → IF TARGET_MODEL==grok → !grok_heavy.md GROK_HANDSHAKE.TRIGGER_2 (генерим ПОД Grok: строгий JSON)
