...
Выпуск про данные и меню: пять пунктов выводились, а модулей за ними не было.
...
- **GPT-6 Astra** (`gpt-6-astra`, GA 09.09): 1 050 000 / 128 000, $10 / $1 cached / $12.50 write /
  ...
  Primary для `webdev / frontend_generation`, fallback `claude-fable-5-1 (max)`.
...
### Fixed
- **Пункты меню больше не ведут в пустоту.** `[24] CONTRACT BUILDER` → `!pipeline.md`
  (и там 11 шагов, как в самом модуле, а не 9 из подписи), `[25] EXPLORATION MODE` →
  `!sandbox.md`, `[19] MENTOR METHOD` → `!!db_v8H.md §11`, `[18] KB BROWSER` → `_index_v8H.md`
  (стоял `_index_v8A.md` — файл донорской редакции). Модули `!visual.md` и `!art.md`,
  которых в поставке не было вовсе, добавлены — `[15] VISUAL CODING` и арт-баннеры работают.
- **Подписи агентов `[7]` и `[9]` приведены к `!agents.md`**: ANON — исполнение инструментов
  и neutral reviewer, VECTOR — данные и аналитика. Безопасность живёт в `!security.md [39]`.
- **Алиасы `-latest` убраны** (22 штуки): `gemini-3.1-pro-preview` вместо
  `gemini-3.1-pro-latest`, а `gemini-3.1-flash-latest` вообще не существовал как id.
- **Реестр API-строк вычищен:** снят задвоенный `kimi-k3`, снят фантом `moonshot-v2-8k`,
  добавлены `glm-5.2` / `glm-5.1` / `glm-5.1-flash` и `gemini-3.8-flash` / `3.7-flash`.
- **Маршрут swarm — `kimi-k2.6`** вместо `moonshot-v2-128k`, которого нет ни в одном реестре.
- **Маршрут agentic — `claude-opus-4-8`** вместо legacy `gpt-5.5`.
...
### Changed
- Рекомендации по задачам (`MODEL_ROUTING_BY_TASK`, `RECOMMENDATIONS`) пересобраны по срезу
  04.09: код — Opus 5 и Fable 5.1, агенты — Opus 5, бесплатный уровень — Sonnet 5.
  Прежний блок был датирован 02.05 и советовал Opus 4.7 и Sonnet 4.6.
- Счётчики дедлайнов пересчитаны от 10.09; истёкшее промо GLM-5.3-Flash переведено в исполненные.
