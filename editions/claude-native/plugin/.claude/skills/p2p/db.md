...
# P2P v8C.3-ALPHA — БАЗА ЗНАНИЙ (db.md)
...
### G6 — OPUS47_TOKENIZER_INFLATION #DB_ERROR_G6
**Модель:** Claude Opus 4.7  
**Симптом:** Контекст расходуется быстрее, чем ожидается  
**Причина:** Токенизатор Opus 4.7 даёт +10-35% inflation vs Opus 4.6  
**Fix:** Планируй ~160K effective context max (не 200K). Cost-sensitive → Opus 4.6.
...
### G10 — GPT55_CONTEXT_PRICING_TRAP #DB_ERROR_G10
**Модель:** GPT-5.5  
**Симптом:** Внезапный прыжок стоимости  
**Причина:** Significant pricing jump >272K input tokens  
**Fix:** Держи под 272K. Для >272K → Gemini 3.1 Pro (1M context).
...
**Симптом:** Устаревшие алиасы будут удалены  
**★ DEADLINE: 2026-07-24**  
**Fix:** Обнови на `deepseek-v4-pro` или `deepseek-v4-flash` немедленно.
...
### API Strings (confirmed May 2026) #DB_TECHNIQUE_APISTRINGS

| Модель | API String | Статус |
|--------|-----------|--------|
| Claude Opus 4.7 | `claude-opus-4-7` | ✅ Актуальный |
| Claude Sonnet 4.6 | `claude-sonnet-4-6` | ✅ Актуальный, Free tier |
| Claude Haiku 4.5 | `claude-haiku-4-5-20251001` | ✅ Актуальный |
| Claude Opus 4.6 | `claude-opus-4-6` | ✅ Pinned (long-context) |
...
Прогнать через Type A–P scanner (см. core.md).
...
**Prevention:** Regular context hygiene. memory_bridge.md для state transfer без context carryover.
...
Legal prompt engineering for over-refusal bypass:
...
Связывает debug_engine.md диагностику с contract_builder.md патчингом.

**STEP 1 — DIAGNOSE:**
Route failure description через debug_engine.md symptom_diagnosis.
...
> Для быстрой маршрутизации без полной загрузки intent_engine.md
