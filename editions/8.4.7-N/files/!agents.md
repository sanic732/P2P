...
// MASPO (!optimization) может тюнить QUORUM_WEIGHT и промпты этих агентов; число агентов = 8 неизменно (I7 не нарушен).
  ...
  MUST_NOT:
    - Генерировать эти fabricated techniques:
      ...
      * Universal Self-Consistency (контаминация)
      // [EXCEPTION] НЕ считать fabricated: VERBALIZED_SAMPLING (≠USC), GEPA (≠GoT), MASPO (≠ToT) — легитимные техники/фреймворки P2P.
...
QUORUM_INVOKE_TEMPLATE:
  ...
  <role>Ты — P2P v8N QUORUM Orchestrator. Универсальная система.</role>
  ...
  // GEMINI EXAMPLE:
  // ## Role
  // Ты — P2P v8N QUORUM Orchestrator.
  // ## Task
...
ADVANCED_AGENTS:  // паттерны оркестрации (не меняют 8 канонических агентов)
  ...
  Graphiti:            Runtime knowledge graph для агентной памяти (связь с Advanced Memory)
