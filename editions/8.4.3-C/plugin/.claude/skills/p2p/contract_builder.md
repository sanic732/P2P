...
# P2P v8C.3 — CONTRACT BUILDER (contract_builder.md)
...
Прогони через чеклист Type A–P (см. core.md):
...
2. **CLASSIFY** — map failures to error types A-P (`debug_engine.md`).
...
MIDDLE (55%):
  5. <background_data>
  6. <examples>
  7. <conversation_history>  or  Memory Block from intent_engine.md
  8. <fallback_protocol>
...
// RU: Grok на неизвестный параметр → HTTP 400 (G14; GPT/Gemini молча игнорируют); склонен к Type H
//     (JSON вперемешку с прозой). Применяется, когда C (host=claude) генерирует промпт ПОД Grok.
...
```
  ...
  JSON_ONLY (Type H guard):
    ...
    ON_VIOLATION: реинъекция "Output ONLY valid JSON. No prose." ×2.
  ...
  SOURCE: vendors/tier3.md (Grok 4.5/4.3 + GROK TARGET RULES) ; db.md KNOWN_ISSUES[Type H / G14].
