...
> Compatible with v8 SIR Scanner in `core.md` — extends, never replaces.
...
- Question limit is PILOT-aware (see pilot_mode in core.md):
...
PROJECT_CARD integration: if loaded in `core.md`, auto-fill dim 6 (stack), 4 (constraints), 7 (audience), and expand glossary terms across all dimensions.

---

## §2. TOOL ROUTING — 12 CATEGORIES

| # | Category | Routing rule | Template (from `templates_library.md` / `db.md`) |
|---|----------|-------------|-------------------------------------------------|
...
| 8 | IMAGE AI (MJ/DALL-E/SD/Flux) | MJ: comma descriptors + `--ar --v 6`; DALL-E: prose + "no text"; SD: `(weight)` + mandatory negative | Visual (see `visual_suite.md`) |
| 9 | VIDEO AI (Sora/Veo/Runway/Kling) | Camera movement + duration sec + cut style + subject continuity | Visual adapted |
| 10 | VOICE AI (ElevenLabs/Suno/Udio) | Emotion + pacing + emphasis markers + speech rate (parameters, not prose) | see `visual_suite.md` §3 |
...
Source priority:
1. `memory_bridge.md` CAPSULE (if loaded).
2. PROJECT_CARD in `core.md`.
