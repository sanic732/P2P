...
# P2P — CONTRACT BUILDER (!contract.md)
...
3. Сверься с `_live/live_vendors.md` (BASE-слой сборки): context window, output limit, цена, G-errors target-модели.
...
Источник метрик — `_live/*` на дату LAST_VERIFIED; свежее приходит LIVE-каналом по `/p2p-download`
...
// RU: Grok на неизвестный параметр → HTTP 400 (G14; GPT/Gemini молча игнорируют). Плюс склонен к
//     Type H (JSON вперемешку с прозой). Отсюда — жёсткий конверт. Применяется, когда C (host=claude)
//     генерирует промпт ПОД Grok. Полный Heavy-16 пак (8 агентов) — эксклюзив High/Light.

GROK_JSON_TARGET:
  WHEN: TARGET_MODEL == grok (генерация промпта ПОД Grok).
...
```
  ...
  JSON_ONLY (Type H guard):
    ...
    MUST NOT:  проза до/после/между JSON; markdown-обёртки вне ```json; trailing commas; одинарные кавычки; NaN/Infinity.
