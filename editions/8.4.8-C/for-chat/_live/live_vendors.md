...
> Full live specs: LIVE-канал по `/p2p-download` (gist `a64245c3…/raw/live_specs.md`, PRIORITY: OVERRIDE) — в сборку не вшит  
...
#   У SpaceXAI порог устроен иначе (200K, удваивается и кэш) — одна общая заглушка два случая не описывает.
...
|---|---|---|---|
| SpaceXAI (grok-4.5) | 200K | ×2 input, ×2 output | ⚠ кэш ТОЖЕ ×2 — кэширование не спасает, резать контекст |
| OpenAI (Sol) | 272K | ×2 input, ×2 cached, ×1.5 output | ❌ cached НЕ освобождён — исправлено 8.4.7 |
| Anthropic / Google | порога не опубликовано | — | — |

> Перехват: SpaceXAI — на 190K, жёсткий обрыв 195K. OpenAI — перехват 250K, обрыв 260K, и решать
