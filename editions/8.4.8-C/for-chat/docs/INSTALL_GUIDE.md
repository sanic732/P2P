...
A: Обычно ничего делать не нужно — свежее приходит по `/p2p-download` из LIVE-гиста.
Если автор раздал файл отдельно, положить его рядом и сказать «старт»: он перекроет BASE по
правилу OVERRIDE (его `VERSION` новее, чем `LAST_VERIFIED` сборки). База v8C.3 не меняется.
