...
**Ключевая деталь (исправлено 8.4.7):** cached input **НЕ освобождён** — прежняя запись «cached
EXEMPT» была ошибкой. По официальной странице цен множитель ×2 применяется и к нему, поэтому
стабильный префикс от подорожания за порогом не спасает.  
...
⚠ У SpaceXAI порог устроен ИНАЧЕ: 200K, и там удваивается также кэш (см. G14).
...
⚠ **Порог удорожания SpaceXAI (не путать с G10).** От 200K из 500K контекста: $2/$0.30/$6 → $4/$0.60/$12.
...
как output. Структурная причина дрейфа стоимости на агентных циклах.
...
к хранилищу секретов — без явного allowlist и журнала аудита. Риск возникает везде, где у Sol есть
право записи, а не только там, где его оценивают.
...
Прогнать через Type A–Q scanner (см. core.md).
