...
Выпуск про данные: сборка перестала называть отменённые цены и исполненные дедлайны
как будущие. Плагинная форма догнала чат-форму — до этого она несла живой слой от 26.07.

### Факты 8.7.4 (срез 2026-09-10)
- **GPT-6 Astra** (`gpt-6-astra`, GA 09.09): 1 050 000 / 128 000, $10 / $1 cached / $12.50 write /
  $50, cutoff 2026-04-30. Порог 272K действует вместе с кэшем — контекст держать ниже.
  Инструменты только через `/v1/responses` (на chat-completions с `reasoning_effort` — 400).
  Маршрут webdev на Astra в этой редакции — только по явному указанию оператора
  (operator-explicit), как и `claude-fable-5-1`.
- **Вендор Grok — SpaceXAI (бывш. xAI)**; имя продукта и api-строки прежние, распознавание хоста
  понимает оба написания.
- **GLM-5.3-Flash** $0.15 / $0.03 / $0.50 (промо истекло 09.09); **Qwen3.8-Max** max output
  131 072; **MiniMax Token Plan** Plus $22 / Max $55 / Ultra $132 (track-only).
- **Claude Code 2.1.267**: `maxEffortLevel`, хуки `PreModelSwitch`, `--system-prompt-snapshot off`,
  пол версии Fable 5.1 — v2.1.257, коды 404/400 вместо 403.
- **G1 переписан**: `temperature` / `top_p` / `top_k` deprecated для всей линии Gemini 3.x
  с 21.07.2026; Deep Think — без temperature, глубина `thinking_level`.
- **Gemini 3.5 Pro** — пятый пропуск GA; **Grok 4.7** не маршрутизируется (нет id и цены);
  **deepseek-v4.1-flash** в BASE не вносится.
- **G22 и Astra**: запрет Sol-образца не переносится и G22 не снимается — проверка на уровне
  результата (диффы, прогоны, ворота разрешений), а не чтением рассуждений.

### Что было сломано у нас и как исправлено
- Перезалив 8.4.7-L с CRLF 10.09 сломал загрузку модулей на части хостов; ассет возвращён
  к опубликованному в тот же день. В 8.4.8 сборка идёт из тега, концы строк проверяет карантин.
- Прежний чек-лист выпуска не имел проверяемых пунктов — публикацию теперь запрещает
  `tools/release_gate.py` с парой-слепотой.
- Вшитый `live_specs.md` убран из поставки: факты в BASE, свежее — по LIVE-каналу.
- Модули Lite — отдельный гист на каждый выпуск.
- У уже установленного 8.4.7 новые маршруты по LIVE-каналу не появятся: DELTA несёт цены,
  статусы и дедлайны, но не таблицу маршрутов.
...
- **Плагин больше не отстаёт от чат-формы.** Живой слой (`vendors/_live_*.md`) был
  на полтора месяца старше и нёс три факта, отменённых эталоном: «cached input EXEMPT»,
  запрет strict-JSON у Qwen3.8-Max, «Fable 5 SUSPENDED globally». Внутри одного дерева
  один файл объявлял основной моделью Opus 4.8, другой — Opus 5.
- **Ссылки внутри плагина ведут туда, где файл лежит.** Головной `core.md` объявлял
  зависимости по именам чат-формы — ни один из четырёх путей в дереве плагина не существовал;
  всего таких ссылок было 59.
- **Пункт `[42] Создать Agent Skill` заработал** — модуля `skills.md` в плагине не было.
- **G3 (Grok topic drift) описан** — стоял «(RESERVED)», хотя Grok в редакции поддерживается.
- **Маршрут «супер-дёшево» больше не ведёт на MiniMax-M3** — эта модель track-only:
  P2P на ней работает, но не маршрутизирует на неё. Fallback — `glm-5.1-flash`.
- **Computer use на `gpt-5.6-sol` помечен G22** — нужен явный allowlist и журнал аудита.
- **Маршрут кода — `claude-opus-5`** (был Opus 4.8), глубокий анализ — Opus 5 → Sol.

### Changed
- `SKILL.md` объявляет платформу Opus 5 / Fable 5.1 / Sonnet 5 вместо Fable 5 / Opus 4.8 / Sonnet 4.6.
- Таблица «контекст → модель» больше не занижает Opus 5 и Sonnet 5 до 200K.

## [8.4.7-C] — 2026-09-05

Изменения с версии 8.4.6-C. Обе формы поставки (`for-chat` и `plugin`) синхронны.

### Removed
- **Оптическое сжатие `/p2p-pxpipe` (L-OPTICAL) — удалено целиком.** Скилл, команда, гайд,
  блок в `compression`, `PXPIPE_GATE` в `agents`, optical-backend CAPSULE в `memory_bridge`,
  Type Q сканера анти-паттернов (сканер стал A–P). Замены нет: остаются LLMLingua,
  Gist Tokens, Verbatim Deletion, Selective Summarization и CAPSULE.
  Поставка похудела на 68 МБ.

### Changed
- **Live specs 8.7.3** (срез 2026-09-04) вместо 8.7.2 от 26.07: Fable 5.1 GA, четыре
  исполненных дедлайна, девять новых, обновлённые цены и статусы восьми вендоров.
- **`bb4pda` — нейтральная редакция.** Без фирменного стиля: механика форума и ограничения
  парсера. Покрывает не только лонгрид, но и ответ в теме, объявление, раздел шапки,
  цитату с пермалинком, обращение к модератору.

### Fixed
- **`claude-sonnet-4-6` больше не помечен снятым.** Модель активна, снятие не раньше
  17.02.2027; 30.06 сменился только дефолт. Поколение 4.6+ — законный выбор по цене.
- **Цена Sonnet 5 — $2/$10.** Объявленное подорожание до $3/$15 с 01.09 отменено 10.08.
