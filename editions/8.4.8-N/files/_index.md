...
LOAD_ORDER:
  REQUIRED: 1 → 2 → 3 → 4 → 5 → 6 (+ LIVE OVERRIDE по /p2p-download, если есть сеть)
...
MACROS:
  ...
  // ON-DEMAND модули:
  /p2p-rag | /p2p-reasoning | /p2p-route | /p2p-compress | /p2p-security | /p2p-optimize |
  /p2p-skill → [26-32]

// ─── EXTENSIONS ANCHOR (динамические пункты [26-32] — правило в !!core_v8N.md §4) ───
// Модули грузятся по триггеру ИЛИ MODULE_*=true|or, по умолчанию скрыты.
// Отображение в меню — ЕДИНОЕ, в статичном ядре core §4 (всегда в памяти, не отдельный файл).
EXTENSIONS_ANCHOR:
  DISPLAY_RULE:   !!core_v8N.md §4 → MENU_DISPLAY_RULE (источник истины)
  TRIGGERS_MENU:  /start | start | старт | /p2p | /menu
  EXT_TRIGGERS:   rag | reasoning | routing | compression | security | optimization | skill | модуль
  MODULE_MAP (файл → пункт): !rag.md→[26] · !reasoning.md→[27] · !routing.md→[28] ·
                             !compression.md→[29] · !security.md→[30] · !optimization.md→[31] ·
                             !skills.md→[32]
  RULE: пункт [26-32] показывается, если MODULE_*=true|or ИЛИ сработал триггер модуля.
        Выбран пункт, а тела файла в контексте нет → сказать «приложи !<module>.md»
        и НЕ выдумывать содержимое модуля.
