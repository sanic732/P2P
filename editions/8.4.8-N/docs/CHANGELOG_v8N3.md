...
## [8.4.1 · 2026-07-06] Меню-гейт по факту загрузки + нативный автодетект хоста (10)
- **Меню отражает реально загруженные модули.** `!!core_v8N.md §4`: `EXTENSIONS_SCAN` + `AVAILABILITY` +
  `MENU_RENDER_ALGORITHM`. Пункты [26-31] печатаются рабочими, только если тело файла-модуля в контексте;
  иначе — в 🔒-футере. Флаг `MODULE_*=true` без файла → пункт остаётся LOCKED. Устранён баг двойного «26».
- **Хост:** `HOST_MODEL_AUTODETECT` (SELF_IDENTIFY + CONFIDENCE_GATE, `HOST_MODEL=""`), **10 хостов**
  (+minimax +manus); `HOST_PICK_LIST [1..10]` — ручной выбор, когда автодетект не сработал (Qwen).
  Хост определяется/выбирается ПЕРЕД меню. Подробности — `HOST_GUIDE.md`, `FAQ_И_ОШИБКИ.md`.
- **Решение по архитектуре:** эксперимент base/extended-сплита отброшен — экономию токенов даёт само
  приложение подмножества файлов; логика гейта вшита СТАТИЧНО в ядро (всегда в памяти модели).
