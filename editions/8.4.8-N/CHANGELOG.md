...
Выпуск про данные: цены, дедлайны и маршруты приведены к срезу 04.09.
...
### Fixed
- **Алиасы `-latest` убраны** (13 штук): `gemini-3.1-pro-preview` вместо `-latest`,
  `gemini-3.1-flash-latest` как id не существовал — маршрут «быстрый T0-1» ведёт
  на `gemini-3.8-flash`.
- **Маршрут swarm — `kimi-k2.6`** вместо несуществующего `moonshot-v2-128k`.
- **Маршрут agentic — `claude-opus-4-8`** вместо legacy `gpt-5.5`.
- **`gemini-3.8-flash` и `3.7-flash` внесены в реестр** — bulk-primary был объявлен,
  но маршрутизатор до него не доходил.
...
### Changed
- Рекомендации по задачам пересобраны по срезу 04.09 (блок был датирован 02.05):
  код — Opus 5, агенты — Opus 5, бесплатный уровень — Sonnet 5, а не Sonnet 4.6.
- Счётчики дедлайнов пересчитаны от 10.09.
