...
### CAPSULE optical-backend (Code + Fable 5 — новое в v8C.3)
Когда CAPSULE крупный (нарратив/история ≥8000 симв) и ENV=Code и model=Fable 5:
```
1. Разделить CAPSULE: HEAD (byte-exact — id, пути, флаги, PROJECT_CARD) остаётся .md текстом;
   BODY (нарратив истории/решений) → PNG через L-OPTICAL.
2. node .claude/skills/pxpipe/compress.mjs capsule_body.md --reader claude-fable-5
   --out .claude/state/pxpipe_cache
3. Сохранить capsule_[name].md = HEAD-текст + ссылки на PNG (BODY) + byte-exact sidecar текстом.
4. Восстановление: модель читает PNG-страницы BODY + HEAD/sidecar как текст.
```
Гейты те же (PXPIPE_GATE). Если PXPIPE_PROXY_ACTIVE → backend OFF (прокси уже сжал).
Замер: measure.mjs. Экономия на теле CAPSULE замерена ~81–83%.
