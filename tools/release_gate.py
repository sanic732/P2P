...
# pack_release.py объявляет EXCLUDE = ("pack.", ".plugin", ".zip") и проверяет
# f.endswith(EXCLUDE) — «pack.» не срабатывает НИКОГДА, потому что имя файла
# на «pack.» не кончается. pack.ps1 и pack.sh уезжают внутрь плагина.
...
def g03_числа_тел(о: Отчёт, м: Мишень, g: Git, ver: str, ref: str, ref_живой: bool) -> None:
    ...
    p.ожидалось = 5
    состав = (lambda pref: g.paths_at(ref, pref)) if ref_живой else (lambda pref: g.ls_files(pref))
...
def g04_мусор(о: Отчёт, м: Мишень, ver: str) -> None:
    ...
    for ассет in list(м.редакции.values()) + м.плагины:
        for имя in записи_архива(ассет):
            p.осмотр()
            if МУСОР_ИМЕНА.search(имя):
                p.провал(f"{ассет.name}: служебный файл сборки внутри поставки — {имя}")
            elif МУСОР_КАТАЛОГИ.search(имя):
                p.провал(f"{ассет.name}: служебный каталог внутри поставки — {имя}")
            elif МУСОР_РАСШИРЕНИЯ.search(имя):
                p.провал(f"{ассет.name}: вложенный архив/временный файл — {имя}")
