# p2p-clean.ps1 — полная очистка остатков плагина/маркетплейса P2P в Claude Code.
# Безопасно: делает бэкап и удаляет ТОЛЬКО записи P2P (claude-plugins-official и др. не трогает).
# Запуск:  правый клик → "Выполнить с помощью PowerShell"
#   либо:  powershell -NoProfile -ExecutionPolicy Bypass -File p2p-clean.ps1
...
try {
  ...
  # Запись считается "P2P", если ключ начинается с p2p ИЛИ относится к кривому десктоп-маркетплейсу
